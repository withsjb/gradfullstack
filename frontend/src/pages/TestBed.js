...
function App() {
  ...
  return (
   ...
   <div className={Styles.tbedbody}>
      ...
      <ul className={Styles.tbedul}>
        {links.map((link) => (
          <li className={Styles.tbedli} key={link._id}>
            <a className={Styles.tbedlink} href={link.url} target="_blank" rel="noopener noreferrer">
              {link.title}
            </a>
            {/* 수정 버튼 */}
            <button className={Styles.teditbtn}
              onClick={() => setEditLink(
            ...
            {/* 삭제 버튼 */}
            <button className={Styles.tdeletebtn} 
            onClick={() => handleDeleteLink(link._id)}>삭제</button>
          </li>
        ))}
      </ul>
      ...
      {/* 새 링크 입력 필드 */}
      <input className={Styles.linkaddtilte}
        type="text"
        placeholder="제목"
        value={newLink.title}
        onChange={(e) => setNewLink({ ...newLink, title: e.target.value })} /><br></br>
      <input className={Styles.linkaddtext}
        type="text"
        placeholder="URL"
        value={newLink.url}
        onChange={(e) => setNewLink({ ...newLink, url: e.target.value })} /> <br></br>
      <button className={Styles.linkaddbtn}  onClick={handleAddLink}> Add Link <i className={Styles.icon}> 
