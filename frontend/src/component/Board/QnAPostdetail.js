...
function PostDetail() {
    ...
    async function fetchPostAndComments() {
      try {
        const postResponse = await axios.get(
          `http://localhost:4000/qnaboard/${id}`
        );
        const commentsResponse = await axios.get(
          `http://localhost:4000/qnaboard/${id}/comments`
        );
  ...
  const handleLike = async () => {
    try {
      await axios.post(`http://localhost:4000/qnaboard/${id}/like`);
  ...
  const handleCommentSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.post(`http://localhost:4000/qnaboard/${id}/comments`, {
        text: comment,
      });
      setComment("");
